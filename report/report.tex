...
\begin{description}
    \item[HarvestHQ]\cite{harvesthq_chosen} it changes the default style of a \verb%multiselect% in something with more visually attractive features,
    \item[jQueryUI, TimePicker]\cite{timepicker} \cite{jqueryui} they improve the default \verb%datepicker% of jQueryUI in a more detailed one (you can set even the second)
    \item[jQuery, flotcharts]\cite{jquery} ~\cite{flotchart} They both are jQuery plugins specialized to create plots.
\end{description}
...
Using graphs is a great way to visually compare encoders, but we think numbers are just not enough, we want more visual feedback of the progress.
In that we are measuring video quality with respect to the human perceptions, thus what a better way to just display side by side these videos for a human to compare for himself?
That's why we took the visual comparator SplitView~\cite{splitview} (developed originally by one of our member) and plug it directly into our interface.
The principle of SplitView is simple as you can see~\ref{fig:split1}: two videos are rendered into the same area, thus they are overlapping.
They are split in the middle by a vertical bar which can moved to unveil part of the video of your choice.
Thus anyone can compare regions of the same sample and inspect the visual differences due to the different encoding settings.
They are multiple samples and they can be chosen with the \verb%comboboxes% above them.

Not only did we use SplitView but we contributed a new important feature: this is the \emph{Blind Test} which is enabled by the green button at the top of the videos in figure~\ref{fig:split1}.
When this is clicked, the green box with video details becomes hidden and the two videos are swapped randomly.
The goal of this feature is to avoid any bias due to the user knowing prior information about the two videos.
Thus the two videos, now shuffled, must be distinguished only by their quality as the eye perceive it, and not by some prior judgement.
Of course you can show the video details again by pressing the green button again to reveal you were right or not.
%Thanks to this feature you loose completely the notion of which would be the best one based on his intrinsic value. When you reclick on the blind test button, the solution reappears.
...
Right now, it's easy for an user to change the set of videos that is used for
...
instead the \verb%cron_enco.py% script has to be modified manually, this requires direct access to the Orchestra machine. The modification wouldn't be too hard to allow basic editing but we lacked the time to implement a well integrated solution.
