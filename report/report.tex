...
\begin{description}
    \item[HarvestHQ]\cite{harvesthq_chosen} it changes the default style of a
      \verb%multiselect% in something with more visually attractive features,
    \item[jQueryUI, TimePicker]\cite{timepicker} \cite{jqueryui} they improve
      the default \verb%datepicker% of jQueryUI in a more detailed one (you can set
      even the second)
    \item[jQuery, flotcharts]\cite{jquery} ~\cite{flotchart} They both are
      jQuery plugins specialized to create plots.
\end{description}
...
Using graphs is a great way to visually compare encoders, but we think numbers
are just not enough, we want more visual feedback of the progress.  In that we
are measuring video quality with respect to the human perceptions, thus what a
better way to just display side by side these videos for a human to compare for
himself?  That's why we took the visual comparator SplitView~\cite{splitview}
(developed originally by one of our member) and plug it directly into our
...
settings.  They are multiple samples and they can be chosen with the
\verb%comboboxes% above them.

Not only did we use SplitView but we contributed a new important feature: this
is the \emph{Blind Test} which is enabled by the green button at the top of the
videos in figure~\ref{fig:split1}.  When this is clicked, the green box with
video details becomes hidden and the two videos are swapped randomly.  The goal
...
again to reveal you were right or not.
...
OSCIED was tested on Ubuntu 14.04 with \verb%1.18.2%. One should read the deployment guide\cite{oscied_deploy} (but with our repository) and take a look at the thesis\cite{df_thesis}. We deployed locally in LXC and the following steps should help anyone to install our system:
\begin{verbatim}
